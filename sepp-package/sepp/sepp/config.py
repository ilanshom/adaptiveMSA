...
# uym2 added #
    ...
    otherGroup.add_argument(
        ...
        dest="kmer_size", metavar="N",
        default=9,
        help="kmer size considered when assigning fragments to subsets. [default: 10]")
    otherGroup.add_argument(
        "-kmerstc", "--kmers_to_check", type=int,
        dest="kmers_to_check", metavar="N",
        default=40,
        help="how many kmers from the query secuence should be used to estimate Jaccard similarity. [default: 20]")
    otherGroup.add_argument(
        "-setstc", "--top_sets_to_check", type=int,
        dest="top_sets_to_check", metavar="N",
        default=10,
        help="how many of the top scoring sets to calculate Jaccard similiarity on. [default: 5]")
    otherGroup.add_argument(
        "-srnds", "--sample_rounds", type=int,
        dest="sample_rounds", metavar="N",
        default=2,
        help="how many times should a batch of kmers be sampled from query seqeunce and checked in top half of remaining candidate hmms. [default: 1]")
    otherGroup.add_argument(
        "-iscr", "--use_intersection_score",
        dest="int_score", 
        default=False, action='store_true',
        help="use weighted intersection instead of weighted Jaccard similarity as quality score. [default: %(default)s]")
    otherGroup.add_argument(
        "-exact", "--exact_computation",
        dest="exact", 
        default=False, action='store_true',
        help="compute weighted Jaccard similarity for every query HMM pair instead of using estimation. [default: %(default)s]")  
    otherGroup.add_argument(
        "-choose_K", 
        dest="choose_K", 
        default=False, action='store_true',
    ...
    otherGroup.add_argument(
        "-Ks_to_check",
        dest="Ks_to_check", metavar="N",
        default="5,10",
        help="the K values to test when choosing K. Seperate numbers by commas without spaces e.g. 5,10 [default: 5,10]]")
    otherGroup.add_argument(
        "-seqs_unmatched_thresh", type=float,
        dest="seqs_unmatched_thresh", metavar="N",
        default=0.1,
        help="the smallest fraction of query sequences should be unmatched in order for algotihm to use a small K. [default: 0.1]")
