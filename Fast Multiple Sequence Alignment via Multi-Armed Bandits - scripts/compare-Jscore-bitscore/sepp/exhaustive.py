...
class QuerySetJob(Job):
    ...
    def run(self):
        ...
        return (query_assignment, query_set_comparison)
...
class JoinFastSearchJobs(Join):
    ...
    def perform(self):
        ...
        for fragment_chunk_problem in self.root_problem.iter_leaves():
            align_problem = fragment_chunk_problem.get_parent()
            i1 = align_problems.index(align_problem)
            search_res = fragment_chunk_problem.get_job_result_by_name(
                "hmmsearch")
            for key, val in search_res.items():    
        ...
        for fc in fc_jobs:
            ...
            fc_job_kmer_scores = fc_job.result[1]
            for qseq in fc_job_kmer_scores:
                kmer_scores[qseq] = {}
                for set1 in fc_job_kmer_scores[qseq]:
                    kmer_scores[qseq][set1] = fc_job_kmer_scores[qseq][set1]
            
        
        
        both_scores = {}
        both_scores_list = []
        for qseq in e_values:
            both_scores[qseq] = {}
            for set1 in e_values[qseq]:
                both_scores[qseq][set1] = (e_values[qseq][set1][1], kmer_scores[qseq][set1][2])
                both_scores_list.append((e_values[qseq][set1][1], kmer_scores[qseq][set1][2]))

        print("")
        print("")
        print("")
        print("")                
        print("both_scores, e-value, k-mer score:")
        print(both_scores)
        print("")
        print("")
        print("")
        print("")
        print(both_scores_list)
        print("")
        print("")
        print("")
        print("")        
